...
where $\theta$ is difference of mean in two groups and $\sigma$ is standard deviation of $\theta$.
...
`stats::power.t.test()` uses the t-distribution for test statistics that is recommended in practice.
...
- Under logrank test with piecewise exponential distribution and
  1:1 randomization ratio, it can be simplified to
