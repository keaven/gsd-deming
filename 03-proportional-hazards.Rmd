...
The number of events drives power, regardless of study duration, as noted by @Schoenfeld; that approximation for event requirements is provided in the `nSurv()` function for fixed design and `gsSurv()` for group sequential design as can be seen by code generated in the app or in **gsDesign** package documentation.
