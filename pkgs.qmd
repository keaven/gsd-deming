# Introduction to gsdmvn, gsDesign2, and simtrial {#sec-pkgs}

```{r, setup, file="_common.R", include=FALSE}
```

simtrial, gsDesign2, and gsdmvn are under development and are hosted on GitHub.
Below are the GitHub repository links to the source code:
...
::: {.callout-note}
As of June 14, 2023, the gsdmvn package has been integrated into the
...
- Average hazard ratio model (see detailed description in @sec-secAhr)
  - `gs_power_ahr()`: Power computation
  - `gs_design_ahr()`: Design computations
- Weighted logrank model (see detailed description in @sec-wlr)
  - `gs_power_wlr()`: Power computation
  - `gs_design_wlr()`: Design computations
- MaxCombo model (see detailed description in @sec-maxcombo)
...
The details of this simulation can be found in @sec-AhrSimSimfix.
...
The details of this simulation can be found in @sec-AhrSimSimPWSurv.

The third simulation is to compare the estimation of $\beta$ by MLE and weighted summation as in @sec-secAhr.
The details of this simulation can be found in @sec-AhrSimMleWs.

### Simulation 1: Compare the IA power by `gs_power_ahr()` and simulation `sim_fix()` {#sec-AhrSimSimfix}
...
### Simulation 2: Compare AHR by `gsDesign2::AHR()` and `simtrial::simPWSurv()` {#sec-AhrSimSimPWSurv}
...
### Simulation 3: Compare $\beta$ by MLE and weighted summation {#sec-AhrSimMleWs}
