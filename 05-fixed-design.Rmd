...
These assumptions will be used unless other clarification is made in specific section.
:::

## Notation

We also define some commonly used notation as below.
...
The key step to calculate sample size is to define the effect size.
...
Let's revisit two sample t-test to make connection between math formula and R code.
...
::: {.rmdnote}
As an exercise, the readers can derive the effect size in formula (9.3) of the
Section 9.5 for the [NCSU ST520 course notes](https://www4.stat.ncsu.edu/~dzhang2/st520/520notes.pdf).
:::
...
::: {.rmdnote}
For simplicity, we skip the detail of sample size calculation,
...
Under proportional hazard assumption, we commonly used event driven approach for sample size calculation.
(i.e. calculate number of events then derive the sample size.)

- Event ($d$) to Sample size ($n$) or `d->n`

```{r, echo = FALSE, out.width= "100%"}
knitr::include_graphics("images/ch2_d_to_n.png")
```
...
```{r, echo = FALSE, out.width= "100%"}
knitr::include_graphics("images/ch2_n_to_d.png")
```
...
The figure below illustrated the survival probability over time in two treatment groups.
...
For weighted logrank test, we first illustrated it by using the Fleming-Harrington weight.
...
- Taylor expansion builds connection between $\Delta$ and weighted average of log hazard ratio (AHR).
  It is a generalization of the @schoenfeld1981asymptotic asymptotic expansion.
...
To calculate sample size for the MaxCombo test,
