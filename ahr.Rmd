...
In this section, we define the the average hazard ratio (AHR) and use it  to derive the asymptotic normal distribution of the logrank test.
...
$$
  ...
  \right)^{-1}.
$$
By plugging the above weights into equation \@ref(eq:AhrBetaEstimation), $\beta$ can be estimated as
$$
  \tilde\beta
  =
  \frac{
...
By plugging the asymptotic distribution of $\hat\beta_m$ in equation \@ref(eq:AhrSignleLogHazardRatio), one gets the asymptotic distribution of $\tilde\beta$ as
...
$$
where $$\mathcal{I} = \sum_{m = 1}^M \left( \frac{1}{E(d_{0,m})} + \frac{1}{E(d_{1,m})} \right)^{-1}.$$
...
In this example, we investigate the correlation between AHR and power, which can be found in the following figure.

```{r, warning=FALSE, message=FALSE}
ggplot(tibble(AHR = seq(.6, 1, .02), 
...
In this example, we investigate the expected accrual of endpoints. 
The setting of this example is very simular to that in Example 1, and we are interested to figure out the expected events per 100 enrolled. 
...
Given the above setting, we know the interim timing, final timing, effect size, and information. 
```{r, echo=FALSE}
ahr <- gsDesign2::AHR(
...
We will discuss the symmetric design and assymetric design in Section XXX and Section XXX, respectively.
...
Second, we calculate the the sample size and crossing probability under non-proportional hazard.
...
In this example, we discuss the design with interims at specified times, i.e., there are futulity boundary only at the first interim analysis (look for p=0.05 in the wrong direction) and there are efficacy boundaries only after the first interim.
