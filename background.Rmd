...
Non-proportional hazards (NPH) is very common to see when there is a delayed effect.
...
And the test statistics is usually the Z-process and B-process, which are used to monitor the treatment effect under group sequential design.
If a Z-process (B-process) cross the boundary, the the trail is stopped.
...
The first one is so called the error spending approach, which specifies boundary crossing probabilities at each analysis.
This is most commonly done with the error spending function approach [@LanDeMets].
The second one is so called the boundary family approach, which specifies how big boundary values should be relative to each other and adjust these relative values by a constant multiple to control overall error rates.
...
In the remaining of this section, we will introduce the above spending functions one by one.

### Haybittle boundary

The main idea of haybittle boundary is to set the interim Z-score boundary as 3 and the final boundary as 1.96, which is visualized as the figure below.
...
The first modified version uses Bonferroni adjustment: for the first $K-1$ analyses, significant p-value is set as 0.001; and for the final analysis, significant p-value is set as $0.05 - 0.01 \times (K-1)$.
...
The second modified version sets the Z-score as 4 for the first half of the study and set the Z-score boundary as 3 thereafter.

The third modified version requires crossing the boundary at two successive looks.

In summary, Haybittle boundary uses piecewise constant boundary, which make it very easy to implement.
...
Its main weakness is the high price for the end of the trial and requirement of equally spaced looks.

### O'Brien-Fleming boundary

O'Brien-Fleming boundary is very conservative at the beginning, which gives realtively large boundary first.
But it gives nominal value close to the overall value of the design $\approx 1.96$ for the final stage.
...
Here is an example of O'Brien-Fleming boundary.
...
Although both Pocock boundary and OBF boundary are belonging to Wang-Tsiatis boundary's family.
But there are two differences between them.

The first difference is that, the Pocock boundary has flat Z-score boundaries; while the O'Brien-Fleming has decreasing Z-score boundary.
...
In conclusion, we visualize the three spending function in the following figure.
...
The treatment effect under survival outcome is usually measured by log rank test, i.e.,
$$
  \widehat\delta(\tau)
  =
