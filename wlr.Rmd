...
We can calculate sample size required for a group sequential design by solving
...
::: {.rmdnote}
For simplicity, we illustrate the sample size and power calculation based on the boundary from logrank test.
...
The key difference is that we considered 2 interim analysis at 12 and 24 month before the final analysis at 36 month.
...
In `gsdmvn`, the sample size can be calculated using `gsdmvn::gs_design_wlr()` for WLR test.
For comparison purposes, we also provided the sample size calculation using `gsdmvn::gs_design_ahr()` for logrank test.
...
- `info` column is $\sigma^2_k$ times $N$

```{r}
n_subject * sigma2
```

- `info0` column is $\sigma^2_k$ times $N$ under the null where we both use `arm0` for calculation in active and control group.
...
- `info0` column is $\sigma^2_k$ times $N$ under the null.
